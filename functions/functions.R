  ...
  for (i in 1:length(inference)){
    ...
    if (plot==TRUE){
      ...
      plot(x=x,y=y,xaxt='n',yaxt='n',xlab='',ylab='',bty='n',xlim=c(0,90),ylim=c(0,90),
           pch=19,col='#2c7fb88C')
      axis(1,at=seq(0,90,10),cex.axis=1.4,pos=-3)
      axis(2,at=seq(0,90,10),cex.axis=1.4,las=2,pos=-3)
      mtext(paste0(inference[i],' predicted VO2'),1,line=4,cex=1.5)
...
# Compare VO2
compare_linear_vo2_bwh <- function(data,truth,inference,indicator,write_out=FALSE,
                                   out_path=NULL,plot=FALSE){
  out <- list()
  for (i in 1:length(inference)){
    y <- data[,get(truth)]
    ...
    if (plot==TRUE){
      pdf(paste0(out_path,inference[i],'_',indicator,'.pdf'),height=4,width=4,pointsize=5)
      par(mar=c(5,5,2,2),oma=c(1,1,1,1))
      plot(x=x,y=y,xaxt='n',yaxt='n',xlab='',ylab='',bty='n',xlim=c(0,50),ylim=c(0,50),
           pch=19,col='#2c7fb88C')
      axis(1,at=seq(0,50,10),cex.axis=1.4)
      axis(2,at=seq(0,50,10),cex.axis=1.4,las=2)
      mtext(paste0(inference[i],' predicted VO2'),1,line=4,cex=1.5)
      mtext('True VO2 (ml/kg/min)',2,line=3.2,cex=1.5)
      text(40,10,labels=paste0('r=',round(cor$estimate,2),' (',round(cor$conf.int[1],2),
                               ' - ',round(cor$conf.int[2],2),')'),cex=1.2)
      text(40,5,labels=paste0('MAE=',round(mae,2)),cex=1.2)
